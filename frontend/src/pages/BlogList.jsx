...
export default function BlogList() {
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-slate-50 dark:bg-darkBg pt-32 pb-24 relative overflow-hidden text-slate-900 dark:text-white w-full">
        {/* Subtle Brand Decor */}
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 w-full">
              ...
              <button
                ...
                className={`px-6 py-2.5 rounded-full text-xs font-black uppercase tracking-widest transition-all duration-300 border ${
                  activeCat === cat 
                    ...
                    : "bg-slate-200/50 dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:text-brandBlue dark:hover:text-white hover:border-brandBlue/30 dark:hover:border-white/20"
                }`}
              >
                {cat}
              </button>
          ...
          {featured && activeCat === "All" && (
            ...
            >
              <Link to={`/blog/${featured.slug}`} className="block relative rounded-[3rem] overflow-hidden border border-slate-200 dark:border-white/5 bg-slate-100 dark:bg-brandNavy shadow-xl dark:shadow-2xl aspect-[21/9]">
                <img 
                  src={featured.img} 
                  alt={featured.title} 
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-white via-white/80 dark:from-darkBg dark:via-darkBg/60 to-transparent flex flex-col justify-end p-10 md:p-16">
                  ...
                  <h2 className="text-4xl md:text-6xl font-black text-slate-900 dark:text-white mb-6 leading-tight tracking-tight group-hover:text-brandBlue transition-colors">
                    {featured.title}
                  </h2>
                  <p className="text-slate-600 dark:text-slate-300 text-lg md:text-xl max-w-3xl line-clamp-2">
                    {featured.excerpt}
                  </p>
                </div>
              </Link>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {(activeCat === "All" ? gridPosts : filteredPosts).map((post, i) => (
              ...
              >
                <Link to={`/blog/${post.slug}`} className="flex-1 glass-card border border-slate-200 dark:border-white/5 rounded-[2.5rem] p-8 hover:border-brandBlue/20 transition-all duration-500 hover:bg-slate-50 dark:hover:bg-brandNavy/30 flex flex-col items-start shadow-sm dark:shadow-xl">
                  <div className="relative w-full rounded-3xl overflow-hidden mb-8 aspect-video border border-slate-200 dark:border-white/10 shadow-md dark:shadow-lg">
                    <img 
                      src={post.img} 
                      alt={post.title} 
                    ...
                    />
                    <div className="absolute top-4 left-4 bg-white/90 dark:bg-brandNavy/80 backdrop-blur-md text-[10px] text-brandBlue font-black uppercase tracking-[0.2em] px-4 py-2 rounded-xl border border-slate-200 dark:border-white/5 shadow-sm dark:shadow-2xl">
                      {post.category}
                    </div>
                  </div>
                  ...
                  <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-4 tracking-tight group-hover:text-brandBlue transition-colors leading-tight">
                    {post.title}
                  </h3>
                  
                  <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed line-clamp-3 mb-8">
                    {post.excerpt}
                  </p>

                  <div className="mt-auto flex items-center gap-2 text-xs font-black text-slate-900 dark:text-white uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all transform translate-x-[-10px] group-hover:translate-x-0">
          ...
          {filteredPosts.length === 0 && (
            <div className="py-40 text-center flex flex-col items-center">
              ...
              <p className="text-slate-600 dark:text-slate-500 font-black uppercase tracking-widest">No Intelligence Entries Available</p>
            </div>
          )}
